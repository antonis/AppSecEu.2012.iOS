...
- (void) loadData {
	...
	NSString *dataPath = [documentsDirectory stringByAppendingPathComponent:@".appdata.plist"];
    ...
    self.speakers = [self.data objectForKey:@"speakers"];
    self.speaches = [self.data objectForKey:@"speaches"];
    self.tracks = [self.data objectForKey:@"tracks"];
    NSLog(@"loadData: speakers=%d, tracks=%d, speaches=%d",[self.speakers count],[self.speaches count],[self.tracks count]);
}
